...
<div>
	...
	<input type="button" value="로그인" onclick="location.href='../user/login.do';">
...
<table border="1">
	<tr>
		<td>			
			<c:choose>
				<c:when test="${param.orderCond == 'boardno_asc' }">
					<a href="mypage.do?orderCond=boardno_desc">번호</a>	
				</c:when>
				<c:when test="${param.orderCond == 'boardno_desc' }">
					<a href="mypage.do?orderCond=boardno_asc">번호</a>	
				</c:when>
				<c:otherwise>
					<a href="mypage.do?orderCond=boardno_desc">번호</a>
				</c:otherwise>
...
<input type="button" value="전체 보기" onclick="location.href='/spring/board/index.do';">
</body>
</html>